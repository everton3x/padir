...
#' @import readr
#' @import stringr
parse_funcao <- function(arquivo_txt){
  ...
  df <- read_fwf(
    ...
    col_types = cols(
      ...
      codigo_funcao = col_character(),
      nome_funcao = col_character()
    ),
    skip = 1,
    trim_ws = T,
    progress = T,
  ...
  # Formata campos
  df$nome_funcao <- str_trim(df$nome_funcao)
